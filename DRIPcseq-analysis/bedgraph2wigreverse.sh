...
grep "^chr" ${S} | cut -f1 | sort -u > ${x%%.sorted.nodup.reverse.q30.bedgraph.sortd}rchr.list
cat ${x%%.sorted.nodup.reverse.q30.bedgraph.sortd}rchr.list | while read C
do
...
rm ${x%%.sorted.nodup.reverse.q30.bedgraph.sortd}rchr.list
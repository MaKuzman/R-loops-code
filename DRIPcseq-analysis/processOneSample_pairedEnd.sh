...
# ----------------Run this script from a directory containing links to both ends of DRIPc paired end sequencing experiment----------------- #
...
y=`ls *fastq.gz | cut -d "_" -f1 | sort |uniq`
qsub -v IN1=${y}_1.fastq.gz,IN2=${y}_2.fastq.gz,OUT1=${y}_1_TRIMMED.fastq,OUT2=${y}_2_TRIMMED.fastq,OUT_SINGLE=${y}_singles_TRIMMED.fastq -N trim${y##DRIP} 1_bbduk.sh
...
qsub -W depend=afterok:$pid -v GENOME=$GENOME,IN1=${y}_1_TRIMMED.fastq,IN2=${y}_2_TRIMMED.fastq,OUT=${y} -N map${y##DRIP} 2_Mapping.sh
pid2=`qstat | grep map${y##DRIP} | awk  {'print $1'}`


# ----------------Sam to sorted bam removed duplicates----------------- #
qsub -W depend=afterok:$pid2 -v IN=$y -N s2sb${y##DRIP} /common/WORK/mfabijanic/programs/samToSortedBam.sh
